...
class BaseAnalyzerTests(TestCase):
    ...
    def setUp(self):
        ...
        self.analyzer = BaseAnalyzer(self.code, self.example_project)

    def test_init(self):
        self.assertEqual(self.analyzer.surround_by, 2)
        self.assertEqual(self.analyzer.code, self.code)
        self.assertEqual(self.analyzer.source, self.example_project)
    ...
    def test_analyze_file(self):
        with self.assertRaises(NotImplementedError):
            self.analyzer.analyze_file(*self.code.items()[0])

    def test_analyze(self):
        results = CustomAnalyzer(self.code, self.example_project).analyze()
